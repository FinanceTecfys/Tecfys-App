...
describe("formatExportCell", () => {
  ...
  it("prints an em dash for empty values", () => {
    ...
    for (const key of ["cancelDate", "additionalStatus", "settlementAmount", "outstanding", "extensionMonths"]) {
...
describe("loanBookToXlsx", () => {
  it("writes a sheet with the header, one row per contract and a totals row", async () => {
    const rows = [row(), row({ id: "id-2", contractNumber: "LB-2", cancelDate: "2026-01-15", additionalStatus: "CAP", settlementAmount: 1800, lifecycleStatus: "Finished", outstanding: 0 })];
    ...
    expect(ws.getRow(4).getCell(1).value).toBe("LB-16535");
    expect(ws.getRow(4).getCell(5).value).toBe("ES");
    // Dates are real dates and amounts real numbers, so Excel can filter and sum them.
    expect(ws.getRow(4).getCell(8).value).toBeInstanceOf(Date);
    expect(ws.getRow(4).getCell(12).value).toBeCloseTo(116058.68, 6);
    expect(ws.getRow(5).getCell(17).value).toBe(1800);

    const totals = ws.getRow(6);
    expect(totals.getCell(1).value).toBe("TOTAL");
    expect(totals.getCell(12).value).toBeCloseTo(116058.68 * 2, 6);
    expect(totals.getCell(17).value).toBe(1800);
    expect(totals.getCell(18).value).toBeCloseTo(37760.05, 6);
  });
